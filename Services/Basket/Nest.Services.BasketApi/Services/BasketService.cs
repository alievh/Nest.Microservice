...
public class BasketService : IBasketService
{
    ...
    {
        var existBasket = await _redisService.GetBasketDb().StringGetAsync(userId);
    ...
    {
        var status = await _redisService.GetBasketDb().StringSetAsync(basketDto.UserId, JsonSerializer.Serialize(basketDto));
    ...
    {
        var status = await _redisService.GetBasketDb().KeyDeleteAsync(userId);
